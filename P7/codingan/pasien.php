...
img.foto { width:60px; height:60px; border-radius:50%; object-fit:cover; }
</style>
</head>
<body>
...
<?php
$query = mysqli_query($conn,"SELECT * FROM pasien ORDER BY id_pasien ASC");
if(mysqli_num_rows($query) > 0){
while($data = mysqli_fetch_assoc($query)){

    // ===== FIX FOTO =====
    $folder_upload = "../uploads/";
    $folder_tampil = "../uploads/";

    if(!empty($data['foto']) && file_exists($folder_upload.$data['foto'])){
        $foto = $folder_tampil.$data['foto'];
    } else {
        $foto = $folder_tampil."default.png";
    }
?>
<tr>
    ...
    <td class="actions">
        <a class="edit" href="edit_data.php?id=<?= $data['id_pasien'] ?>">Edit</a>
        <a class="hapus" href="pasien.php?hapus=<?= $data['id_pasien'] ?>" onclick="return confirm('Hapus data ini?')">Hapus</a>
    </td>
</tr>
<?php }} else { ?>
<tr><td colspan="9"><i>Belum ada data</i></td></tr>
<?php } ?>
</table>
